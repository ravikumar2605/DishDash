...
const Cart = () => {
  const { isCartOpen: isOpen, closeCart: onClose, cartItems, updateQuantity: onUpdateQuantity, removeFromCart: onRemoveItem } = useCart();
  ...
  // Full-page cart is now a separate route/component
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-primary-600 to-secondary-600">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-white text-opacity-90">{itemCount} items</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => { onClose(); navigate('/cart'); }}
              className="px-3 py-2 rounded-lg bg-white/20 hover:bg-white/30 text-white text-sm transition-colors duration-200"
            >
              View Full Cart
            </button>
            <button
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-6">
                <div className="space-y-4">
                  {cartItems.map((item) => (
                    <div key={`${item.restaurantId}-${item.name}`} className="bg-gray-50 rounded-xl p-4 hover:bg-gray-100 transition-colors duration-200">
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex flex-col items-end space-y-2">
                          <div className="flex items-center space-x-2">
                            <button
                              onClick={() => handleQuantityChange(`${item.restaurantId}-${item.name}`, item.quantity - 1)}
                          ...
                          {/* Remove Button */}
                          <button
                            onClick={() => onRemoveItem(`${item.restaurantId}-${item.name}`)}
              ...
              <div className="border-t border-gray-200 p-6 bg-gray-50">
                ...
                <div className="mt-4 p-3 bg-green-50 rounded-lg">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-green-700 font-medium">Free delivery on orders over $25</span>
                  </div>
                </div>
              </div>
            </>
